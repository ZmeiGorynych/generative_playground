...
reward_fun = lambda x: scorer(x) + np.array([- 5*max(x-5,0) for x in num_aromatic_rings(x)])# - 0.2*np.array([0 if num is None else num for num in num_aromatic_rings(x)])# + reward_aliphatic_rings(x)# + 0.05*reward_aromatic_rings(x)#lambda x: reward_aromatic_rings(x)# #lambda x: reward_aromatic_rings(x)#
...
model, fitter1, fitter2 = train_policy_gradient(molecules,
                                                ...
                                                plot_prefix='anchor cycle',
                                                dashboard='anchor wweak sa',#None,#
                                                save_file='paper/policy_gradient_anchor_wweak_sa20_cycle.h5',
                                                smiles_save_file='paper/pg_smiles_anchor_wweak_sa20_cycle.h5',
