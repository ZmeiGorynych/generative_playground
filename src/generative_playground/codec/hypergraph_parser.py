...
def abstract_ring_atom(graph: HyperGraph, loc):
    ...
    neighbors = [(edge_id, graph.other_end_of_edge(loc, edge_id)) for edge_id in atom.edge_ids]
    ...
    internal_neighbor_ids = [x for x in neighbors if x[1] not in graph.child_ids() # '
    ...
    if len(internal_neighbor_ids) >= 2 and len(internal_neighbor_ids) < len(neighbors):
        ...
        child_edge_ids = [old_new_edge_map[edge_id] if edge_id in old_new_edge_map else edge_id
        ...
        replacement_node = Node(edge_ids=replace_edge_ids,
...
def check_tree_top_level(new_tree):
    ...
    for child, child_id in zip(new_tree, new_tree.node.child_ids()):
        assert len(child.node.node[child.node.parent_node_id].edge_ids) == \
               len(new_tree.node.node[child_id].edge_ids)
...
def graph_from_graph_tree(tree):
    '''
    This constructs the graph directly from graph tree, bypassing the conversion to a linear rules list
    :param tree:
    :return:
    '''
