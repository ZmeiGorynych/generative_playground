...
class Mutator:
    ...
    def __call__(self, model, pick_best=64, total_probs=True):
        print('mutating!')
        internal_model = model.model.stepper.model
        internal_model.collapse_unconditionals()
        model_biases = internal_model.conditionals.data

        log_priors = internal_model.grammar.get_all_conditional_log_frequencies()
        if self.log_priors is None:
            self.log_priors = torch.from_numpy(log_priors).to(dtype=model_biases.dtype,
                                                              device=model_biases.device)
            self.log_priors.requires_grad=False

        if total_probs:
            log_probs = self.log_priors + model_biases
        else:
            log_probs = model_biases

        # pick the two rows to swap...
        max_probs = log_probs.max(dim=1)[0] # really? Not axis =1?
        best = torch.argsort(max_probs, descending=True )[:pick_best]
        ...
        # need to include the biases, too, so the TOTAL new probs are swapped
        dprior = self.log_priors[orig_inds[0]] - self.log_priors[orig_inds[1]]
        cond_slice = internal_model.conditionals.data[orig_inds[0]].clone()
        internal_model.conditionals.data[orig_inds[0]] = internal_model.conditionals.data[orig_inds[1]] - dprior
        internal_model.conditionals.data[orig_inds[1]] = cond_slice + dprior
        return model
